...
public class GifDecoder {
    ...
    public int readOneFrame(InputStream is) throws IOException {
        init();
        if (is != null) {
            if (!(is instanceof BufferedInputStream))
                is = new BufferedInputStream(is);
    ...
    protected void readContentsOneFrame() {
        // read GIF file content blocks
        boolean done = false;
        while (!(done || err())) {
            int code = read();
    ...
    protected void readLSD() throws IOException {
        ...
        if (width > OfflineAuth.maxPngDimension || height > OfflineAuth.maxPngDimension) {
